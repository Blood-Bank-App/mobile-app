...
export function ModeProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadMode = useCallback(async () => {
    try {
      ...
      if (profile?.mode) {
        const profileMode = profile.mode as AppMode;
        setModeState(profileMode);
        // Also save to local storage for consistency
        await AsyncStorage.setItem('app-mode', profileMode);
        console.log('✅ ModeProvider: Mode synced from profile:', profileMode);
      } else {
  ...
  const toggleMode = useCallback(() => {
    setMode(mode === 'patient' ? 'donor' : 'patient');
  }, [mode, setMode]);

  const refreshMode = useCallback(async () => {
    console.log('🔄 ModeProvider: Refreshing mode from profile...');
    setIsLoading(true);
    await loadMode();
  }, [loadMode]);

  const value = useMemo(
    () => ({ mode, setMode, toggleMode, refreshMode, isLoading }),
    [mode, setMode, toggleMode, refreshMode, isLoading]
  );
